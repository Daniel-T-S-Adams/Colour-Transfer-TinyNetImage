...
- Required libs: `numpy`, `scipy`, `Pillow` or `imageio`, `matplotlib`, `pot` Python Optimal Transport
...
### 4. Compute Cost Matrix (using POT) ✅ COMPLETED
Use the Python Optimal Transport (POT) library's efficient `ot.dist()` function to compute the cost matrix.

**Task**:
- Use `C = ot.dist(X_src, X_tgt, metric='sqeuclidean')`. ✅ DONE
- This function computes the squared Euclidean distance `||X_src[i]-X_tgt[j]||^2` between all pairs of points. ✅ DONE
- Our previous POT exploration showed this is fast (~0.3s) and memory-efficient (~64MB) for our 4096x4096 problem. ✅ DONE

**Outputs**:
- Cost matrix `C` of shape `(4096, 4096)` in `float32` or `float64`. ✅ DONE

### 5. Compute Optimal Transport Plan (using POT) ✅ COMPLETED
Use the POT library's `ot.sinkhorn()` function to compute the entropic regularized transport plan.

**Task**:
- Use `P = ot.sinkhorn(w_src, w_tgt, C, reg)`, where `reg` is the epsilon `ε`. ✅ DONE
- Based on our exploration, a good value is `reg=0.01`, which should take about 30 seconds to compute. ✅ DONE
- The `ot.sinkhorn` function is numerically stable and suitable for our use case. ✅ DONE

**Outputs**:
- Transport plan `P`, a `(4096, 4096)` matrix. ✅ DONE
- Diagnostics on convergence and timing for validation. ✅ DONE

### 6. Colour Transfer Using the Transport Plan ✅ COMPLETED
Two common methods:

**(A) Barycentric Mapping (deterministic)** ✅ COMPLETED
...
**(B) Stochastic Sampling** ❌ NOT COMPLETED
For each i, sample j from categorical distribution proportional to `P_{ij}` and assign `X_tgt[j]`.

**Task**:
- Implement both; compare smoothness vs. noise. ✅ COMPLETED (Barycentric done)
- Clip results to `[0,1]`, reshape back to `(64,64,3)`, convert to `uint8` for saving. ✅ DONE

**Outputs**:
- Recoloured image(s) saved to disk. ✅ DONE
- Side-by-side plot: original source, target, transferred. ✅ DONE



### 7. Packaging & Reproducibility ❌ NOT COMPLETED
